...
class IELM(BaseEstimator):
    def __init__(self, numInputNodes, numOutputNodes, numHiddenNodes, forgettingFactor=0.999,
                 lambda_value=0.01):
        """
    ...
    def fit(self, X, y):
        ...
        self._M = (1 / self.forgettingFactor) * self._M - np.dot((1 / self.forgettingFactor) * self._M,
                                                                 np.dot(Ht, np.dot(
                                                                     pinv(np.eye(numSamples) + np.dot(H, np.dot(
                                                                         (1 / self.forgettingFactor) * self._M, Ht))),
